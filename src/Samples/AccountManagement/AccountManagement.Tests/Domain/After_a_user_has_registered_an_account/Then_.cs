...
namespace AccountManagement.Tests.Domain.After_a_user_has_registered_an_account
{
    [TestFixture] public class Then_ : DomainTestBase
    {
        ...
        {
            _registerAccountScenario = new RegisterAccountScenario(ServerBusSession);
        ...
        {
            var result = new LoginScenario(ServerBusSession, _registeredAccount, _registerAccountScenario.Password).Execute();
        ...
        [Test] public void Login_with_the_correct_email_but_wrong_password_fails()
            => new LoginScenario(ServerBusSession, _registeredAccount, "SomeOtherPassword").Execute()
               .Succeeded.Should().Be(false);

        [Test] public void Login_with_the_wrong_email_but_correct_password_fails()
            => new LoginScenario(ServerBusSession, "some_other@email.com", _registerAccountScenario.Password).Execute()
               .Succeeded.Should().Be(false);

        [Test]
        public void Attempting_to_register_an_account_with_the_new_email_fails_with_email_already_registered_message()
        {
            var scenario = new RegisterAccountScenario(ServerBusSession, email: _registerAccountScenario.Email);
