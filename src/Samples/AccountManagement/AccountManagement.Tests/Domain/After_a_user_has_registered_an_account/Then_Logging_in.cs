...
namespace AccountManagement.Tests.Domain.After_a_user_has_registered_an_account
{
    [TestFixture] public class Then_Logging_in : DomainTestBase
    {
        ...
        {
            var result = new LoginScenario(ClientEndpoint, _registerAccountScenario.Email, _registerAccountScenario.Password).Execute();
        ...
        [Test] public void with_the_correct_email_but_wrong_password_fails()
            => new LoginScenario(ClientEndpoint, _registerAccountScenario.Email, "SomeOtherPassword").Execute()
