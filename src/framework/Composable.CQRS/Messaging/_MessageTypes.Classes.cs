...
#pragma warning restore CA1724 //
        ...
        public static partial class Remotable
        {
            public static partial class AtMostOnce
            {
                ...
                {
                    ///<summary>How and when a <see cref="Command.MessageId"/> is generated is vital to correctly maintain the At most once delivery guarantee. When creating the command in the backend service we must generate a new <see cref="Command.MessageId"/>. When binding the command in a UI we must reuse the <see cref="Command.MessageId"/> generated by the backend. This enum helps make that important distinction explicit in your code.</summary>
                    protected enum DeduplicationIdHandling
                    {
                        ///<summary>When creating the command within the owning handler endpoint a new <see cref="Command.MessageId"/> must be generated. </summary>
                        Create,
                        ///<summary>When binding the command in a UI or when deserializing it is very important to NOT create a new <see cref="Command.MessageId"/> as this would break the At most once delivery guarantee.</summary>
                        Reuse
                    }
                    ...
                    protected Command(DeduplicationIdHandling scenario) => _deduplicationId = scenario == DeduplicationIdHandling.Create ? Guid.NewGuid() : Guid.Empty;
