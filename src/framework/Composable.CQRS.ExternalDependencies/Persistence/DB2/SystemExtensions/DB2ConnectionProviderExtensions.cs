...
namespace Composable.Persistence.DB2.SystemExtensions
{
    static class ComposableDB2ConnectionProviderExtensions
    {
        ...
        public static Task UseCommandAsync(this IDB2ConnectionProvider @this, Func<DB2Command, Task> action) => @this.UseConnectionAsync(async connection => await connection.UseCommandAsync(action).NoMarshalling());
