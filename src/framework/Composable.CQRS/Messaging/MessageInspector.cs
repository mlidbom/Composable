    ...
    static partial class MessageInspector
    {
        ...
        static void AssertValid(Type type)
        {
            lock(SuccessfullyInspectedTypes)
            {
                ...
                if(type.Implements<BusApi.Remotable.IMessage>() && type.Implements<BusApi.StrictlyLocal.IRequireLocalReceiver>()) throw new MessageTypeDesignViolationException($"{type.FullName} implements both {typeof(BusApi.Remotable.IMessage).FullName} and {typeof(BusApi.StrictlyLocal.IRequireLocalReceiver).FullName}.");
