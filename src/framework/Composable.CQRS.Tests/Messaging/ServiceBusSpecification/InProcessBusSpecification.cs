...
namespace Composable.Tests.Messaging.ServiceBusSpecification
{
    public class InProcessBusSpecification : IDisposable
    {
        ...
        public class Given_a_bus : InProcessBusSpecification
        {
            ...
            {
                [Fact] public void Send_new_ACommand_throws_an_Exception() => Bus.Invoking(_ => Bus.PostInProcess(new ACommand())).ShouldThrow<NoHandlerException>();
                [Fact] public void Get_new_AQuery_throws_an_Exception() => Bus.Invoking(_ => Bus.PostInProcess(new ACommand())).ShouldThrow<NoHandlerException>();
            ...
            public class With_registered_handler_for_ACommand : Given_a_bus
            {
                ...
                {
                    Bus.PostInProcess(new ACommand());
            ...
            public class With_registered_handler_for_AQuery : Given_a_bus
            {
                ...
                [Fact] public void Getting_new_AQuery_returns_the_instance_returned_by_the_handler() => Bus.GetInProcess(new AQuery()).Should().Be(_aQueryResult);
            }
