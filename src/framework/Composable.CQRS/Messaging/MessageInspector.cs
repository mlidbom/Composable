...
namespace Composable.Messaging
{
    static class MessageInspector
    {
        ...
        {
            if(message is IRequiresTransactionalSendOperationMessage && Transaction.Current == null) throw new Exception($"{message.GetType().FullName} is {nameof(IRequiresTransactionalSendOperationMessage)} but there is no transaction.");
            if(message is IForbidTransactionalSendOperationMessage && Transaction.Current != null) throw new Exception($"{message.GetType().FullName} is {nameof(IForbidTransactionalSendOperationMessage)} but there is a transaction.");
        ...
        static void AssertValid(Type type)
        {
            lock(SuccessfullyInspectedTypes)
            {
                ...
                if(type.Implements<IRemoteMessage>() && type.Implements<ILocalMessage>()) throw new Exception($"{type.FullName} implements both {typeof(IRemoteMessage)} and {typeof(ILocalMessage)}.");
                if(type.Implements<IRequiresTransactionalSendOperationMessage>() && type.Implements<IForbidTransactionalSendOperationMessage>()) throw new Exception($"{type.FullName} implements both {typeof(IRequiresTransactionalSendOperationMessage)} and {typeof(IForbidTransactionalSendOperationMessage)}.");
