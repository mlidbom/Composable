...
namespace Composable.Messaging.Buses.Implementation
{
    static class TransportMessage
    {
        ...
        internal class InComing
        {
            ...
            public MessageTypes.IMessage DeserializeMessageAndCacheForNextCall()
            {
                if(_message == null)
                {
                    ...
                    Assert.State.Assert(!(_message is MessageTypes.IExactlyOnceMessage actualMessage) || MessageId == actualMessage.MessageId);
                }
                return _message;
            }
            ...
            {
                if(typeof(MessageTypes.IRemotableQuery<object>).IsAssignableFrom(messageType))
                    return TransportMessageType.NonTransactionalQuery;
                if(typeof(MessageTypes.IAtMostOnceCommand<object>).IsAssignableFrom(messageType))
                    return TransportMessageType.AtMostOnceCommandWithReturnValue;
                if(typeof(MessageTypes.IAtMostOnceHypermediaCommand).IsAssignableFrom(messageType))
                    return TransportMessageType.AtMostOnceCommand;
                else if(typeof(MessageTypes.IExactlyOnceEvent).IsAssignableFrom(messageType))
                    return TransportMessageType.ExactlyOnceEvent;
                if(typeof(MessageTypes.IExactlyOnceCommand).IsAssignableFrom(messageType))
        ...
        internal class OutGoing
        {
            ...
            public static OutGoing Create(MessageTypes.IRemotableMessage message, ITypeMapper typeMapper, IRemotableMessageSerializer serializer)
            {
                var messageId = (message as MessageTypes.IAtMostOnceMessage)?.MessageId ?? Guid.NewGuid();
                ...
                return new OutGoing(typeMapper.GetId(message.GetType()), messageId, body, message is MessageTypes.IExactlyOnceMessage);
            }
