...
namespace Composable.Messaging
{
    static class MessageInspector
    {
        ...
        {
            if(message is BusApi.RemoteSupport.ExactlyOnce.IRequireTransactionalSender && Transaction.Current == null) throw new Exception($"{message.GetType().FullName} is {nameof(BusApi.RemoteSupport.ExactlyOnce.IRequireTransactionalSender)} but there is no transaction.");
            if(message is BusApi.IForbidTransactionalRemoteSender && Transaction.Current != null) throw new Exception($"{message.GetType().FullName} is {nameof(BusApi.IForbidTransactionalRemoteSender)} but there is a transaction.");
        ...
        static void AssertValid(Type type)
        {
            lock(SuccessfullyInspectedTypes)
            {
                ...
                if(type.Implements<BusApi.RemoteSupport.ExactlyOnce.IRequireTransactionalSender>() && type.Implements<BusApi.IForbidTransactionalRemoteSender>()) throw new Exception($"{type.FullName} implements both {typeof(BusApi.RemoteSupport.ExactlyOnce.IRequireTransactionalSender)} and {typeof(BusApi.IForbidTransactionalRemoteSender)}.");
