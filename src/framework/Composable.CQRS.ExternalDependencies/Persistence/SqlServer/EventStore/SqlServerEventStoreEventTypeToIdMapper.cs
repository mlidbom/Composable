...
namespace Composable.Persistence.SqlServer.EventStore
{
    class SqlServerEventStoreEventTypeToIdMapper : IEventTypeToIdMapper
    {
        ...
        IdTypeMapping InsertNewType(Type newType) => TransactionScopeCe.SuppressAmbientAndExecuteInNewTransaction(() =>
        {
            ...
            {
                var reader = command.SetCommandText($@"SELECT {SqlServerEventTypeTable.Columns.Id} FROM {SqlServerEventTypeTable.Name} WHERE {SqlServerEventTypeTable.Columns.EventType}=@{SqlServerEventTypeTable.Columns.EventType}")
                                    .AddParameter(SqlServerEventTypeTable.Columns.EventType, _typeMapper.GetId(newType).GuidValue)
                                    .ExecuteReader();
                using(reader)
                {
                    if(reader.Read())
            ...
            using(var command = connection.CreateCommand())
            {
                var insertedTypeIntegerId = command.SetCommandText($@"INSERT {SqlServerEventTypeTable.Name} ( {SqlServerEventTypeTable.Columns.EventType} ) OUTPUT INSERTED.{SqlServerEventTypeTable.Columns.Id} VALUES( @{SqlServerEventTypeTable.Columns.EventType} )")
                                                   .AddParameter(SqlServerEventTypeTable.Columns.EventType, _typeMapper.GetId(newType).GuidValue)
                                                   .ExecuteScalar();
        ...
        IEnumerable<IIdTypeMapping> GetTypes() => TransactionScopeCe.SuppressAmbient(() =>
        {
            ...
            {
                using var command = connection.CreateCommand();
                command.CommandText = $"SELECT {SqlServerEventTypeTable.Columns.Id} , {SqlServerEventTypeTable.Columns.EventType} FROM {SqlServerEventTypeTable.Name}";
