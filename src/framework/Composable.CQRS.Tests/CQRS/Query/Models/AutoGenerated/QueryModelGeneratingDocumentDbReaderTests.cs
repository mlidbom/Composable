        ...
        public void CreateContainer()
        {
            _serviceLocator = DependencyInjectionContainer.CreateServiceLocatorForTesting(
                container => container.Register(
                    Singleton.For<IEventStore>().UsingFactoryMethod(() => new InMemoryEventStore()),
                    Scoped.For<IEventStoreUpdater, IEventStoreReader>().UsingFactoryMethod((IEventstoreEventPublisher eventPublisher, IEventStore eventStore, IUtcTimeTimeSource timeSource, IAggregateTypeValidator aggregateTypeValidator) => new EventStoreUpdater(eventPublisher, eventStore, timeSource, aggregateTypeValidator)),
                    Scoped.For<IDocumentDbReader, IVersioningDocumentDbReader>().UsingFactoryMethod((AccountQueryModelGenerator queryModelGenerator) => new QueryModelGeneratingDocumentDbReader(new[] {queryModelGenerator})),
                    Scoped.For<AccountQueryModelGenerator>().UsingFactoryMethod((IEventStoreReader eventStoreReader) => new AccountQueryModelGenerator(eventStoreReader))
                ));
