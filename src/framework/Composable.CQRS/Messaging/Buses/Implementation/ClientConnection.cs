...
namespace Composable.Messaging.Buses.Implementation
{
    class ClientConnection : IClientConnection
    {
        ...
        public void DispatchIfTransactionCommits(MessagingApi.Remote.ExactlyOnce.IEvent @event) => Transaction.Current.OnCommittedSuccessfully(action: () => _state.WithExclusiveAccess(func: state => DispatchMessage(state, TransportMessage.OutGoing.Create(@event, state.TypeMapper))));

        public void DispatchIfTransactionCommits(MessagingApi.Remote.ExactlyOnce.ICommand command) => Transaction.Current.OnCommittedSuccessfully(action: () => _state.WithExclusiveAccess(func: state => DispatchMessage(state, TransportMessage.OutGoing.Create(command, state.TypeMapper))));

        public async Task<TCommandResult> DispatchIfTransactionCommitsAsync<TCommandResult>(MessagingApi.Remote.ExactlyOnce.ICommand<TCommandResult> command) => (TCommandResult)await _state.WithExclusiveAccess(func: async state =>
        {
