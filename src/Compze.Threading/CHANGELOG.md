...
- Internal async bridging for condition waits: `IAwaitableCriticalSection` gains `_internal` extensions — `TryAwaitOnDedicatedThreadAsync` / `TryAwaitOnThreadPoolThreadAsync` — that park the blocking `TryAwait` on a thread the caller names: a dedicated thread for a wait that can be long (parked pool threads starve the pool), the pool only for a wait known to be brief. Internal deliberately: they belong on the public surface once the API is declared ready for publishing; Tessaging's waiting sends consume them meanwhile (`InternalsVisibleTo`).
